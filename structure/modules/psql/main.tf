...
resource "azurerm_postgresql_flexible_server_dtabase" "psql" {
  name = "${var.name_prefix}-psql-fs-db"
  server_id = azurerm_postgresql_flexible_server.id
  carset = "UTF-8"
  collation = "C"
}
...
resource "azurerm_private_endpoint" "psql" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_kubernetes_cluster.psql.id
  }
}

# Create A Record
resource "azurerm_private_dns_a_record" "psql" {
  name = azurerm_kubernetes_cluster.psql.name
